...
import {ArchetypesHelper, Archetype} from '../helpers/archetypes';
import {CastesHelper, Castes} from '../helpers/castes';
import {HomelandsHelper} from '../helpers/homelands';
import {NaturesHelper, Nature} from '../helpers/natures';
...
export class CharacterSerializer {
    static serialize(character: Character): ICharacterData[] {
        return [
            ...
            { name: "homeland", value: character.region ? character.region : HomelandsHelper.getHomeland(character.homeland).name },
            ...
            { name: "weapons", value: CharacterSerializer.serializeWeapons(character.equipment.concat([character.provenanceWeapon])) },
            { name: "armor", value: CharacterSerializer.serializeArmor(character.equipment) },
            { name: "derived", value: `${CharacterSerializer.serializeResolve(character)},${character.vigour},${character.gold}` },
            ...
            { name: "spells", value: CharacterSerializer.serializeArray(character.spells.concat(character.pettyEnchantments)) },
        ];
    }
        ...
        if (eq) {
            eq.forEach(item => {
                if (EquipmentHelper.getWeaponByName(item)) {
                    ...
                    result += `${weapon.sheetName},${props.range},${EquipmentHelper.weaponSizeToString(props.size)},${props.damage},${props.qualities.join(">")}|`;
                }
            });
        }
