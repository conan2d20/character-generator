  ...
  getStories(caste: Caste) {
    ...
    for (var i = 0; i < this._stories[caste].length; i++) {
      ...
      if (st.name === 'Ambition Rewarded') {
        stories.push(
          new StoryViewModel(st.roll, new StoryModel(st.name + ' (Adept)', st.description, st.trait, st.roll))
        );
        stories.push(
          new StoryViewModel(st.roll, new StoryModel(st.name + ' (Overseer)', st.description, st.trait, st.roll))
        );
        stories.push(
          new StoryViewModel(st.roll, new StoryModel(st.name + ' (High Priest)', st.description, st.trait, st.roll))
        );
      } else {
        ...
        if (story.name === 'Ambition Rewarded') {
          ...
          if (roll <= 7) {
            return new StoryViewModel(
              story.roll,
              new StoryModel(story.name + ' (Adept)', story.description, story.trait, story.roll)
            );
          } else if (roll <= 7) {
            return new StoryViewModel(
              story.roll,
              new StoryModel(story.name + ' (Overseer)', story.description, story.trait, story.roll)
            );
          } else {
            return new StoryViewModel(
              story.roll,
              new StoryModel(story.name + ' (High Priest)', story.description, story.trait, story.roll)
            );
          }
        } else {
          return story;
