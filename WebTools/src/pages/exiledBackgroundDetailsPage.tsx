            ...
            <div className="page">
                ...
                <div className="panel">
                    ...
                    <table className="selection-list">
                        <tbody>
                            <tr>
                                <td><RadioButton groupId="attr" value={bg.reduction[0]} onChanged={val => this.onAttributeSelection(val)} /></td>
                                <td>{Attribute[bg.reduction[0]]}</td>
                                <td>{this._attribute === bg.reduction[0] ? character.attributes[bg.reduction[0]].value - 1 : character.attributes[bg.reduction[0]].value}</td>
                            </tr>
                            <tr>
                                <td><RadioButton groupId="attr" value={bg.reduction[1]} onChanged={val => this.onAttributeSelection(val)} /></td>
                                <td>{Attribute[bg.reduction[1]]}</td>
                                <td>{this._attribute === bg.reduction[1] ? character.attributes[bg.reduction[1]].value - 1 : character.attributes[bg.reduction[1]].value}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
