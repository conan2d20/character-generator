    ...
    const attributes = character.attributes.map((a, i) => {
      if (a.value < 14) {
        return (
          <tr key={i}>
            <td>
              <RadioButton groupId="box" value={'attr-' + a.attribute} onChanged={(val) => this.onSelection(val)} />
            </td>
    ...
    const skills = SkillsHelper.getSkills().map((s, i) => {
      if (character.skills[s].expertise < 2) {
        return (
          <tr key={i}>
            <td>
              <RadioButton groupId="box" value={'skill-' + s} onChanged={(val) => this.onSelection(val)} />
            </td>
            <td>{SkillsHelper.getSkillName(s)}</td>
            <td>
              {this._selected === 'skill-' + s ? character.skills[s].expertise + 2 : character.skills[s].expertise} /{' '}
              {this._selected === 'skill-' + s ? character.skills[s].focus + 2 : character.skills[s].focus}
            </td>
          </tr>
        );
      }
