...
import {AttributeAspect, AttributesHelper, AttributeAspectViewModel} from '../helpers/attributes';
...
class AttributeAspectSelection extends React.Component<IAttributeAspectSelectionProperties, {}> {
    ...
    render() {
        ...
        return (
            <tr>
                <td><div className="selection-header">{aspect.name}</div></td>
                <td>{mandatory}</td>
                <td>{optional}</td>
                <td><RadioButton groupId={group} value={aspect.id} onChanged={() => onSelection(aspect.id) }/></td>
            </tr>
        );
    }
}
...
export class AttributeAspectsSelection extends React.Component<IAttributeAspectsSelectionProperties, {}> {
    ...
    render() {
        const aspects1 = AttributesHelper.getAttributeAspects().map((a, i) => {
            return (<AttributeAspectSelection group="first" aspect={a} onSelection={aspect => this.onFirstAspectSelected(aspect) }/>);
        });

        const aspects2 = AttributesHelper.getAttributeAspects().map((a, i) => {
            return (<AttributeAspectSelection group="second" aspect={a} onSelection={aspect => this.onSecondAspectSelected(aspect) }/>);
        });

        const next = this.aspect1 >= 0 && this.aspect2 >= 0
        ...
        return (
            <div>
                ...
                <br/>
                <div className="panel"><b>FIRST ASPECT</b></div>
                ...
                <br/>
                <div className="panel"><b>SECOND ASPECT</b></div>
                <table className="selection-list">
                    <thead>
                        <tr>
                            <td></td>
