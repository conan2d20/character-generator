...
export class Spells {
  private _petty: SpellModel[] = [
    ...
    new SpellModel(
      'Poisons',
      'Create poisonous powders or gases that can damage, blind or incapacitate',
      0,
      Source.Thief
    ),
  ];
  ...
  private _spells: SpellModel[] = [
    ...
    new SpellModel(
      ...
      'Causing the earth itself to strike against an area or structure.',
      1,
      Source.Core
    ),
  ...
  getPettyEnchantments() {
    return this._petty
      .filter((p) => character.hasSource(p.source))
      .sort((a, b) => {
