    ...
    render() {
        ...
        return (
            <tr>
                ...
                <td><RadioButton groupId="best" value={attribute} onChanged={attr => this.onBestSelected(attr as Attribute) } disabled={isWorst} /></td>
                <td><RadioButton groupId="worst" value={attribute} onChanged={attr => this.onWorstSelected(attr as Attribute) } disabled={isBest} /></td>
                <td><div>{value}</div></td>
            </tr>
        );
    }
...
class OptionalAttributeSelection extends React.Component<IOptionalAttributeSelectionProps, {}> {
    ...
    render() {
        ...
        const radioFirst = index < 2
            ? (<RadioButton groupId="first" value={attribute} onChanged={attr => this.onFirstSelected(attr as Attribute) } />)
            : undefined;

        const radioSecond = index > 1
            ? (<RadioButton groupId="second" value={attribute} onChanged={attr => this.onSecondSelected(attr as Attribute) } />)
            : undefined;
...
export class AttributeAspectDetailsPage extends React.Component<IPageProperties, {}> {
    ...
    render() {
        ...
        const header = (<span>{firstAspect.name} & {secondAspect.name}</span>);
        ...
        const mandatory = mandatoryAttributes.map((a, i) => {
            return (<MandatoryAttributeSelection
                key={i}
        ...
        const optional = optionalAttributes.map((a, i) => {
            ...
            return (<OptionalAttributeSelection
                key={i}
                index={i}
                attribute={a}
