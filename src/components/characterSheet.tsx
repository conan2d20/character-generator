...
class CharacterSheetData {
  private _data: SectionContent[] = [
    new SectionContent(
      ...
      character.region
        ...
        ? HomelandsHelper.getHomeland(character.homeland).name
        : 'None'
    ),
    new SectionContent(
      'ATTRIBUTE ASPECTS',
      character.firstAspect >= 0
        ? AttributesHelper.getAttributeAspect(character.firstAspect).name +
          ' & ' +
          AttributesHelper.getAttributeAspect(character.secondAspect).name
        : 'None'
    ),
    new SectionContent('CASTE', character.caste >= 0 ? CastesHelper.getCaste(character.caste).name : 'None'),
    new SectionContent(
      'ARCHETYPE',
      character.archetype >= 0 ? ArchetypesHelper.getArchetype(character.archetype).name : 'None'
    ),
    new SectionContent('NATURE', character.nature >= 0 ? NaturesHelper.getNature(character.nature).name : 'None'),
    new SectionContent(
      'EDUCATION',
      character.education >= 0 ? EducationsHelper.getEducation(character.education).name : 'None'
    ),
  ...
  private _attributes1: Attribute[] = [Attribute.Agility, Attribute.Awareness, Attribute.Brawn, Attribute.Coordination];

  private _attributes2: Attribute[] = [Attribute.Intelligence, Attribute.Personality, Attribute.Willpower];
    ...
    const attributesAndSkills1 = this._sheetData.firstAttributesSection.map((a, i) => {
      ...
      return (
        <table className="sheet-section" key={i}>
          <tbody>
            <tr>
              <td className="bg-dark-wide" style={{ padding: '4px' }}>
                {AttributesHelper.getAttributeName(a).toLocaleUpperCase()}
              </td>
              <td colSpan={2} className="bg-light border-dark text-dark text-center">
    ...
    return (
      <div id="character-sheet" className="sheet-hidden">
        ...
        <div className={containerClass} id="sheet-container">
          ...
          <div className="sheet-panel">{attributesAndSkills1}</div>
          <div className="sheet-panel">{attributesAndSkills2}</div>
          <div className="sheet-panel">
            <table className="sheet-section">
              <tbody>
                <tr>
                  <td className="bg-dark">TALENTS</td>
                  <td className="bg-light border-dark-nopadding text-dark">{talents}</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table className="sheet-section">
              <tbody>
                <tr>
                  ...
                  <td className="bg-light border-dark text-dark">{equipment}</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table className="sheet-section">
              <tbody>
                <tr>
                  ...
                  <td className="bg-light border-dark text-dark">{languages}</td>
                </tr>
              </tbody>
            </table>
          </div>
